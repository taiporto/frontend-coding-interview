...
describe('<Photo />', () => {
  it('should render the photo', () => {
    ...
    expect(screen.getByTestId('avg-color-box')).toHaveStyle({
      backgroundColor: mockedPhoto.avg_color,
    });
    expect(screen.getByRole('link', { name: 'Portfolio' })).toBeVisible();
    expect(screen.getByRole('link', { name: 'Portfolio' })).toHaveAttribute(
      'href',
      mockedPhoto.photographer_url,
    );
  });
  ...
  describe('when the photo is favorited', () => {
    beforeEach(() => {
      localStorage.setItem(
        `CleverPhoto::${mockedUserName}::favoritePhotos`,
        JSON.stringify([mockedPhoto.id.toString()]),
      );
    });
